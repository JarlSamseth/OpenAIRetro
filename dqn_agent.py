...
class DQNAgent:
    ...
    def train_data(self):
        ...
        next_state = np.zeros(((self.batch_size,) + INPUT_SHAPE))
        ...
        return history, next_state, action, reward, done
    ...
    def train_replay(self, iteration):
        ...
        state, next_state, action, reward, done = self.train_data()
        next_targets = self.target_model.predict([next_state, np.ones((self.batch_size, self.action_size))])
        ...
        result = self.train([state, action_one_hot], target_one_hot)
